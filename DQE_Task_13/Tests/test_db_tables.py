"""
DWH table tests (from Task 6). Needs a real Postgres connection to
dwh_hw_db on localhost:5433 (see conftest.py's db_connection fixture) and
Configs/sql_config.yaml (see the note at the top of that file).
"""
...
def run_check(db_connection, sql_cases, case_name):
    """
    Looks up one named case from the parametrized config, runs its SQL
    through the shared DB connection fixture and asserts the result, with
    each step logged to Allure.
    """
