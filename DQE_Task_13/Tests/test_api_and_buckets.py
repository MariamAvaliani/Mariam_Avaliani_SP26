"""
Task 12 - API testing with requests + pytest
--------------------------------------------
...
Fixtures (test_date, aws_client, gcp_client) now live in the shared
Tests/conftest.py instead of a separate conftest.py, since this framework
has one conftest.py for the whole Tests/ folder.
"""
...
class TestUserPosts:
    """
    JSONPlaceholder is a *fake* REST API used for practice. It accepts POST
    requests and answers with a realistic response (id 101), but it does
    NOT actually save anything on the server. That is why the test does
    not rely on "my POSTs must now exist" - instead it:
