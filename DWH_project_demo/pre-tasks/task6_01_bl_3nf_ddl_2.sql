...
--   2. CE_DATE removed entirely -- a calendar-breakdown table is not
--      needed on this layer at all (DIM_DATES is already built
--      independently of BL_3NF, at BL_DM). CE_TRANSACTION carries
--      TRANSACTION_DATE directly as its own column instead of a DATE_SK
--      lookup.
...
--      customer version.
--
-- REVISION 3 -- mentor feedback on the PR for this script:
--   "Source-based uniqueness constraints based on src_id not source
--   triplet." CE_PRODUCT / CE_STORE / CE_PAYMENT / CE_DELIVERY hold
--   entities that are source-specific (retail and online each run their
--   own independent id sequence for products/stores/payments/deliveries,
--   so the same numeric id can legitimately mean two different
--   real-world things depending on which source it came from). Their
--   UNIQUE constraints are widened from the bare source id to
--   (source_system, <id>) so a coincidental id collision between sources
--   is never merged into one row. CE_CATEGORY / CE_SUBCATEGORY /
--   CE_COUNTRY / CE_REGION are deliberately left on their name-based
--   natural key -- those are conformed dimensions (same real-world
--   category/country regardless of which source mentions it), so
--   merging by name across sources is the intended, documented rule,
--   not a bug. CE_CUSTOMER has no UNIQUE constraint on customer_id to
--   begin with (SCD2 -- several historical rows are expected per
--   customer_id), so there is no constraint to widen here; the
--   equivalent fix for CE_CUSTOMER is in the load logic (script 3),
--   which now matches SCD2 versions per (source_system, customer_id)
--   instead of customer_id alone.
--
...
--             ce_customer (SCD Type 2), ce_payment, ce_delivery
--             ce_transaction (fact, carries transaction_date directly)
...
-- CONFORMED dimension (REVISION 3): the same category name means the same
-- real-world category no matter which source it came from, so the natural
-- key stays CATEGORY_NAME alone, on purpose -- rows from both sources are
-- meant to merge into one here. This is the documented "conformed" path
-- from the mentor's feedback, as opposed to CE_PRODUCT/CE_STORE/
-- CE_PAYMENT/CE_DELIVERY below, which are source-specific.
...
CREATE TABLE IF NOT EXISTS bl_3nf.ce_product (
    ...
    -- widened to (source_system, product_id) -- product_id is only unique
    -- within one source, not across both (REVISION 3)
    CONSTRAINT uq_ce_product_id UNIQUE (source_system, product_id),
...
-- COUNTRY_NAME alone is the natural key on purpose, both sources are meant
-- to merge into one row per country.
...
CREATE TABLE IF NOT EXISTS bl_3nf.ce_store (
    ...
    -- widened to (source_system, store_id) -- same reasoning as CE_PRODUCT
    -- above (REVISION 3)
    CONSTRAINT uq_ce_store_id UNIQUE (source_system, store_id),
...
CREATE TABLE IF NOT EXISTS bl_3nf.ce_payment (
    ...
    -- widened to (source_system, payment_id) -- payment_id is derived from
    -- transaction_id, which is only unique within one source (REVISION 3)
    CONSTRAINT uq_ce_payment_id UNIQUE (source_system, payment_id)
);
...
CREATE TABLE IF NOT EXISTS bl_3nf.ce_delivery (
    ...
    -- widened to (source_system, delivery_id) -- same reasoning as
    -- CE_PAYMENT above (REVISION 3)
    CONSTRAINT uq_ce_delivery_id UNIQUE (source_system, delivery_id)
);
...
-- No CE_DATE table and no DATE_SK column -- a separate calendar table is
-- not needed on the 3NF layer, so TRANSACTION_DATE is stored directly
-- on this table instead of through a date-dimension lookup. DIM_DATES
-- at BL_DM is unaffected -- it was already built independently.
-- REVISION 3: now that CE_PRODUCT/CE_STORE/CE_PAYMENT/CE_DELIVERY allow
-- one row per (source_system, id) instead of one row per id, the surrogate
-- keys product_sk/store_sk/payment_sk/delivery_sk this fact table points
-- at are already resolved to the correct source-specific row at load time
-- (script 3 joins on source_system together with the natural id before
-- picking the *_sk) -- so this table itself needs no structural change,
-- only script 3's load logic changes.
-- ---------------------------------------------------------------------
