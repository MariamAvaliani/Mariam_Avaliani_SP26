...
-- (CE_CATEGORY, CE_SUBCATEGORY, CE_COUNTRY, CE_REGION).
-- REVISION 2.2 -- CE_DATE default row removed. CE_DATE does not exist on
-- my real, deployed BL_3NF schema -- CE_TRANSACTION carries
-- TRANSACTION_DATE directly, so there is no such table to give a
-- default row to.
--
...
-- ---------------------------------------------------------------------
-- CE_DATE -- removed. CE_TRANSACTION stores TRANSACTION_DATE directly as
-- its own column, there is no separate CE_DATE table on my real,
-- deployed BL_3NF schema to give a default row to (see the REVISION 2.1
-- / 2.2 notes in task6_01_bl_3nf_ddl.sql and task6_03_bl_3nf_load.sql).
-- ---------------------------------------------------------------------
